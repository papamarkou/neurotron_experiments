...
# %% Load numerical output of simulation 3
...
# %% Generate and save NeuroTron figure for simulation 3

plt.figure(figsize=[8, 5])

xrange = range(1, tron_error_loaded.shape[0]+1)

labels = [r'$\theta_\ast$ = {}'.format(sim03_setup['boundlist'][i]) for i in range(tron_error_loaded.shape[1])]
...
plt.savefig(
    output_path.joinpath(sim03_setup['name']+'_tron.png'),
    dpi=300,
...
# %% Generate and save SGD figure for simulation 3

plt.figure(figsize=[8, 5])

xrange = range(1, sgd_error_loaded.shape[0]+1)

labels = [r'$\theta_\ast$ = {}'.format(sim03_setup['boundlist'][i]) for i in range(sgd_error_loaded.shape[1])]
...
plt.savefig(
    output_path.joinpath(sim03_setup['name']+'_sgd.png'),
    dpi=300,
    pil_kwargs={'quality': 100},
    transparent=transparent,
    bbox_inches='tight',
...
# %% Generate and save zoomed in NeuroTron figure for simulation 3
...
plt.savefig(
    output_path.joinpath(sim03_setup['name']+'_tron_zoomed.png'),
    dpi=300,
    pil_kwargs={'quality': 100},
    transparent=transparent,
    bbox_inches='tight',
...
# %% Generate and save zoomed in SGD figure for simulation 3
...
plt.savefig(
    output_path.joinpath(sim03_setup['name']+'_sgd_zoomed.png'),
    dpi=300,
    pil_kwargs={'quality': 100},
    transparent=transparent,
    bbox_inches='tight',
...
# %% Generate and save zoomed in NeuroTron-vs-SGD figure for simulation 3
...
for i in range(1, tron_error_loaded.shape[1]):
    ...
    plt.title(r't-distribution data, $\theta_\ast$ = {}'.format(sim03_setup['boundlist'][i]))
    ...
    plt.savefig(
        output_path.joinpath(
            sim03_setup['name']+'_tron_vs_sgd_zoomed_theta_val'+str(i+1).zfill(len(str(tron_error_loaded.shape[1])))+'.png'
        ),
        dpi=300,
        pil_kwargs={'quality': 100},
        transparent=transparent,
