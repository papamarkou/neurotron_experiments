...
for i in range(tron_error_loaded.shape[1]):
    ...
    plt.title(r'Student\'s t data ($df=4$), $\theta_\ast$ = {}'.format(sim08_setup['betalist'][i]))
    ...
    if save:
        plt.savefig(
            output_path.joinpath(
                sim08_setup['name']+'_tron_vs_sgd_theta_val'+str(i+1).zfill(len(str(tron_error_loaded.shape[1])))+'.png'
            ),
            dpi=300,
