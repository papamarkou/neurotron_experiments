...
# %% Generate and save SGD figure for simulation 1

plt.figure(figsize=[8, 5])

xrange = range(1, sgd_error_loaded.shape[0]+1)

labels = [r'$\theta_\ast$ = {}'.format(sim01_setup['boundlist'][i]) for i in range(sgd_error_loaded.shape[1])]
...
# %% Generate and save zoomed in NeuroTron figure for simulation 1
...
# %% Generate and save zoomed in SGD figure for simulation 1
...
# %% Generate and save zoomed in NeuroTron-vs-SGD figure for simulation 1
...
for i in range(1, tron_error_loaded.shape[1]):
    ...
    plt.title(r'Normal data, $\theta_\ast$ = {}'.format(sim01_setup['boundlist'][i]))
